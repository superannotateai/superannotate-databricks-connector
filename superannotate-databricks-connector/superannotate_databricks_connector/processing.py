...
def process_vector_instance(instance, custom_id_map=None):
    ...
    """
    return {'instance_type': instance['type'],
            'classId': instance["classId"] if custom_id_map is None else
            custom_id_map.get(instance["className"]),
            'probability': instance.get('probability'),
            'bbox_points': {k: float(v) for k, v in
